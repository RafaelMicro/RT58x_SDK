...
static void ota_download_handle(uint32_t cmd_id, uint8_t *pBuf)
{
    ...
    else if (cmd_id == 0xF0000001)
    {
        do
        {
            ...
            {
                info("pkt %d / %d\r\n", upg_data->cur_pkt, gt_img_info.total_pkt);
                if (upg_data->cur_pkt == 0)
                {
                    recv_cnt = 0;
                        ...
                        uint16_t *offset)
{
    ...
    if (!find || (plen - idx) < (sizeof(ota_download_cmd_hdr) + sizeof(ota_download_cmd_end)))
    {
        return t_return;
    }
    ...
    *offset = idx;
    ...
    if (cs != end->cs)
    {
        err("Checksum error: expected 0x%x, got 0x%x\n", cs, end->cs);
